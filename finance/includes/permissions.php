...
function _edu_load_permissions(): array
{
 ...
 $current_role = $role ?? $_SESSION['role'] ?? '';
 ...
 if (in_array($current_role, ['dean', 'affairs'])) {
     // In Finance Portal, Deans and Affairs automatically have financial access
     $_edu_permissions_cache = ['financial', 'students', 'results'];
     return $_edu_permissions_cache;
 }
...
function has_permission(string $perm): bool
{
 global $role;
 $current_role = $role ?? $_SESSION['role'] ?? '';
 if (in_array($current_role, ['super_admin', 'admin'])) return true;
...
function require_permission(string $perm): void
{
 ...
 // Redirect to finance index instead of academic dashboard
 $dest = 'index.php';
